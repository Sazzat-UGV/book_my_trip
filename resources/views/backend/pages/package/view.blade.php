...
@endpush
@section('content')
    @include('backend.layout.inc.breadcump', ['page_name' => 'Package View'])
    <div class="container">
        <div class="row">
            <div class="col-12">
                <div class="card mb-4">
                    <div class="card-body">
                        ...
                        <div class="wrapper row">
                            <div class="preview col-md-6">
                                ...
                                <div class="preview-pic tab-content">
                                    <div class="tab-pane active" id="pic-1"><img
                                            src="{{ asset('uploads/package') }}/{{ $packages->package_image }}" />
                                    </div>
                                </div>
                                ...
                                <ul class="preview-thumbnail nav nav-tabs ">
                                    @foreach ($images as $image)
                                        <li class="mb-1"><a data-target="#pic-1" data-toggle="tab"><img
                                                    src="{{ asset('uploads/package') }}/{{ $image->package_multiple_image }}" /></a>
                                        </li>
                                    @endforeach
                                </ul>
                            ...
                            <div class="details col-md-6">
                                <h3 class="product-title">{{ $packages->package_name }}</h3>
                                <div class="rating">
                                    <div class="stars">
                                        @for ($i = 0; $i < $packages->package_rating; $i++)
                                ...
                                <p class="vote"><strong>Validity:
                                    </strong>{{ date('d F Y', strtotime($packages->starting_date)) }} - {{ date('d F Y', strtotime($packages->ending_date)) }}</p>
                                <p class="vote"><strong>From:
                                    </strong>{{ $packages->start_from }}</p>
